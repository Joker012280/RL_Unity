...
class Replay_buffer():
    ...
    def sample(self):
        old_actions, probs, states = torch.FloatTensor().to(device), torch.FloatTensor().to(
...
class Model(nn.Module):
    ...
    def act(self, state):
        ...
        dist = Normal(probs, torch.tensor([0.2]).to(device))
        action = dist.sample()
...
for num_episode in range(max_episode_num):
    ...
    print("# of episode : {}, score : {:.1f}".format(num_episode, \
                                                             total_reward))
    total_reward = 0.0
    ## 결과값 프린트
    if num_episode % save_rate == 0 and num_episode != 0:
        torch.save({
